      ...
      <main className="app-main">
        ...
        <section className="panel panel-left">
          ...
          <div className="canvas-wrapper">
            {/* FIX: SVG responsive + center + bigger */}
            <svg
            ...
            >
              ...
              {config && (
                <>
                  <line x1={Ap.x} y1={Ap.y} x2={Bp.x} y2={Bp.y} className="link link-main" />
                  <line x1={Bp.x} y1={Bp.y} x2={Cp.x} y2={Cp.y} className="link link-main" />
                  <line x1={Cp.x} y1={Cp.y} x2={Dp.x} y2={Dp.y} className="link link-main" />
                  <line x1={Bp.x} y1={Bp.y} x2={Ep.x} y2={Ep.y} className="link link-secondary" />
              ...
              {tracePointsSvg && <polyline points={tracePointsSvg} className="trace" />}
            ...
            <div>
              ...
              <form onSubmit={handleSubmit} className="form-grid">
                <label className="field">
                  <span>l₁ = AB (m)</span>
                  <input type="number" step="0.01" value={l1} onChange={handleParamChange(setL1)} onKeyDown={handleKeyDown} />
                </label>

                <label className="field">
                  <span>l₂ = BC (m)</span>
                  <input type="number" step="0.01" value={l2} onChange={handleParamChange(setL2)} onKeyDown={handleKeyDown} />
                </label>

                <label className="field">
                  <span>l₃ = CD (m)</span>
                  <input type="number" step="0.01" value={l3} onChange={handleParamChange(setL3)} onKeyDown={handleKeyDown} />
                </label>

                <label className="field">
                  <span>x (A → D) (m)</span>
                  <input type="number" step="0.01" value={xd} onChange={handleParamChange(setXd)} onKeyDown={handleKeyDown} />
                </label>

                <label className="field">
                  <span>y (A → D) (m)</span>
                  <input type="number" step="0.01" value={yd} onChange={handleParamChange(setYd)} onKeyDown={handleKeyDown} />
                </label>

                <label className="field">
                  <span>L_BE / L_BC</span>
                  <input type="number" step="0.05" value={lBERatio} onChange={handleParamChange(setLBERatio)} onKeyDown={handleKeyDown} />
                </label>

                <label className="field">
                  <span>∠(BC, BE) (°)</span>
                  <input type="number" step="1" value={alfDeg} onChange={handleParamChange(setAlfDeg)} onKeyDown={handleKeyDown} />
                </label>

                <label className="field">
                  <span>Başlanğıc krank bucağı φ₁ (°)</span>
                  <input type="number" step="1" value={initialFi1Deg} onChange={handleParamChange(setInitialFi1Deg)} onKeyDown={handleKeyDown} />
                </label>
              ...
              {config ? (
                <>
                  ...
                  <div className="result-block">
                    ...
                    <ul>
                      <li>A(0, 0)</li>
                      <li>B({B.x.toFixed(3)}, {B.y.toFixed(3)})</li>
                      <li>C({C.x.toFixed(3)}, {C.y.toFixed(3)})</li>
                      <li>D({D.x.toFixed(3)}, {D.y.toFixed(3)})</li>
                      <li>E({E.x.toFixed(3)}, {E.y.toFixed(3)})</li>
                    </ul>
                  </div>
                  ...
                  <div className="result-block">
                    ...
                    <p>
                      E nöqtəsinin yadda saxlanmış nöqtə sayı: <strong>{tracePoints.length}</strong>
                    </p>
                    <p className="hint">
                      Play düyməsi ilə krank fırlanır, E nöqtəsinin hərəkət yolu qırmızı xətt kimi çəkilir.
                    </p>
                  </div>
                </>
              ) : (
